...
import {
    DownloadCloud, BrainCircuit, TrendingUp, Zap, LayoutList, Target, MessageSquare,
...
const StudentView: React.FC<StudentViewProps> = ({ onNavigate }) => {
    const { lang, user } = useAppContext();
    const [showIDCard, setShowIDCard] = useState(false);
    const isRtl = lang === 'ar';
    // Check active Tab from context if possible or use onNavigate parent logic.
    // Since we are in StudentView, the DashboardLayout handles main navigation. 
    // However, `StudentDashboard` requested `activeTab` prop. The layout logic renders specific components based on activeTab.
    // This View is ONLY rendered when tab is 'overview'. 
    // Wait, DashboardLayout logic for Student:
    // if (activeTab === 'overview') return <StudentView ... />;
    // if (activeTab === 'ai_tutor') ...
    // So this file is ONLY the overview. 
    // The user said "Rewrite StudentDashboard.tsx... Must include Results tab using Cards".
    // If I only rewrite Overview, the Results tab is separate.
    // I must check DashboardLayout.tsx again.
    // DashboardLayout: `if (activeTab === 'exam_results') return <StudentExamModule mode="results" />;`
    // So the Results are handled by `StudentExamModule`.
    // AND `ResultsModule` was asked to be Cards.
    // I will make this View the "Dashboard Overview" and trust that I also updated ResultsModule/StudentExamModule elsewhere? 
    // No, the user logic is "Rewrite StudentDashboard.tsx".
