...
const AdminView: React.FC<{ onNavigate: (t: string) => void }> = ({ onNavigate }) => {
   const { lang, t, allUsers = [], announcements = [], systemName, user } = useAppContext();
   ...
   return (
      <div className="space-y-12 animate-view pb-20">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-8 md:gap-12">
               <div className="space-y-4 md:space-y-6 text-center lg:text-right w-full lg:w-auto">
                  <div className="flex items-center justify-center lg:justify-start gap-3">
                     ...
                     <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-slate-400">{t('branch_console')}</span>
                  </div>
                  <h1 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none">{systemName}</h1>
                  <p className="text-slate-400 font-bold max-w-xl text-base md:text-xl leading-relaxed mx-auto lg:mx-0">{t('admin_welcome_text')}</p>
                  <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-4">
                     <button onClick={() => onNavigate('manage_students')} className="flex-1 sm:flex-none px-6 md:px-8 py-3 md:py-4 bg-white text-black rounded-xl md:rounded-2xl font-black text-xs md:text-sm flex items-center justify-center gap-3 hover:scale-105 transition-all">
                        <Users size={18} md:size={20} /> {t('btn_students_list')}
                     </button>
                     <button onClick={() => onNavigate('approvals')} className="flex-1 sm:flex-none px-6 md:px-8 py-3 md:py-4 bg-indigo-600 text-white rounded-xl md:rounded-2xl font-black text-xs md:text-sm flex items-center justify-center gap-3 hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20">
                        <FileCheck size={18} md:size={20} /> {t('btn_approvals_center')}
                     </button>
                  </div>
               </div>
               ...
               <div className="relative z-10 grid grid-cols-2 gap-4 md:gap-6 w-full lg:w-auto">
                  <div className="p-4 md:p-8 bg-white/5 border border-white/10 rounded-[1.5rem] md:rounded-[2.5rem] backdrop-blur-xl text-center min-w-[120px] md:min-w-[200px] shadow-2xl">
                     <p className="text-[9px] md:text-[11px] font-black text-indigo-400 uppercase mb-2 md:mb-3 tracking-widest">{t('stat_pending_approvals')}</p>
                     <p className="text-3xl md:text-6xl font-black text-white tabular-nums">{pendingApprovals}</p>
                     <button onClick={() => onNavigate('approvals')} className="mt-2 md:mt-4 text-[9px] md:text-[10px] font-black text-slate-400 hover:text-white underline">{t('review_now')}</button>
                  </div>
                  <div className="p-4 md:p-8 bg-white/5 border border-white/10 rounded-[1.5rem] md:rounded-[2.5rem] backdrop-blur-xl text-center min-w-[120px] md:min-w-[200px] shadow-2xl">
                     <p className="text-[9px] md:text-[11px] font-black text-emerald-400 uppercase mb-2 md:mb-3 tracking-widest">{t('stat_general_attendance')}</p>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <AdminStatCard label={t('stat_total_students')} val={studentCount} icon={<Users />} trend="+12" color="text-indigo-600" />
            <AdminStatCard label={t('teacher_name')} val={teacherCount} icon={<GraduationCap />} trend="0" color="text-indigo-600" />
            <AdminStatCard label={t('admin_employees')} val={allUsers.filter(u => u.role === UserRole.ACCOUNTANT || (u.role === UserRole.ADMIN && u.id !== user?.id)).length} icon={<Briefcase />} trend="+1" color="text-slate-900" />
            <AdminStatCard label={t('stat_branch_revenue')} val="12,400" icon={<Landmark />} trend="+8%" color="text-emerald-600" />
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-10">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <AdminTool icon={<UserPlus />} label={t('tool_add_teacher')} onClick={() => onNavigate('manage_teachers')} color="bg-indigo-600" />
                  <AdminTool icon={<Briefcase />} label={t('tool_manage_employees')} onClick={() => onNavigate('manage_employees')} color="bg-indigo-500" />
                  <AdminTool icon={<Users />} label={t('tool_student_affairs')} onClick={() => onNavigate('manage_students')} color="bg-slate-900" />
                  <AdminTool icon={<FileCheck />} label={t('tool_approvals')} onClick={() => onNavigate('approvals')} color="bg-amber-500" />
               </div>
               ...
               <div className="glass-card bg-white p-6 md:p-10 border border-slate-200 rounded-[2rem] md:rounded-[3rem]">
                  <div className="flex flex-col sm:flex-row justify-between items-center mb-6 md:mb-10 border-b pb-6 gap-4">
                     <h3 className="text-xl md:text-2xl font-black flex items-center gap-3 md:gap-4 text-black"><ClipboardCheck className="text-indigo-600" size={24} md:size={28} /> {t('pending_requests')}</h3>
                     <button onClick={() => onNavigate('approvals')} className="w-full sm:w-auto px-6 py-3 bg-slate-100 text-slate-900 rounded-xl text-xs font-black hover:bg-slate-900 hover:text-white transition-all">{t('view_all')}</button>
                  </div>
                        ...
                        <div key={ann.id} className="flex justify-between items-center p-6 bg-slate-50/50 border border-transparent rounded-2xl hover:border-indigo-600 hover:bg-white transition-all group shadow-sm">
                           <div className="flex items-center gap-5">
                              ...
                              <div>
                                 ...
                                 <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">{t('applicant')}: {ann.authorName} ({ann.authorRole})</p>
                              </div>
                           </div>
                           <button onClick={() => onNavigate('approvals')} className="px-6 py-3 bg-white border-2 border-slate-200 text-slate-900 rounded-xl text-xs font-black hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all">{t('preview_approve')}</button>
                        </div>
                     ))}
                     {pendingApprovals === 0 && (
                        <div className="text-center py-20 bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-100">
                           ...
                           <p className="text-slate-400 font-black">{t('no_pending_requests')}</p>
                        </div>
                     )}
                  </div>
               </div>
            ...
            <div className="space-y-10">
               <div className="glass-card p-10 bg-white border border-slate-200 shadow-xl relative overflow-hidden rounded-[3rem]">
                  <h3 className="text-xl font-black mb-10 flex items-center gap-3 text-black">
                     <TrendingUp size={24} className="text-indigo-600" /> {t('growth_analysis')}
                  </h3>
                  <div className="space-y-10">
                     <GrowthBar label={t('academic_performance')} val={88} color="bg-indigo-600" />
                     <GrowthBar label={t('student_expansion')} val={75} color="bg-slate-950" />
                     <GrowthBar label={t('operational_efficiency')} val={92} color="bg-emerald-600" />
                  </div>
                  <div className="mt-12">
                     <SmartAnalytic
                        role={UserRole.ADMIN}
