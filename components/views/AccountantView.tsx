  ...
  return (
    <div className="space-y-12 animate-view pb-24">
         ...
         <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-10">
            <div>
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-xs font-black uppercase tracking-[0.4em] text-indigo-300">Financial Hub Terminal</span>
               </div>
               <h1 className="text-6xl font-black tracking-tighter uppercase italic leading-none mb-6">INTEGRATED ERP</h1>
               <p className="text-slate-400 font-bold text-xl max-w-xl">مرحباً بك في المحرك المالي لمنظومة العقرب. كافة القيود مرحلة والسيولة مراقبة لحظياً.</p>
               <div className="flex gap-4 mt-10">
                  <button onClick={() => onNavigate('acc_vouchers')} className="px-10 py-5 bg-indigo-600 rounded-[1.75rem] font-black text-sm flex items-center gap-3 hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 active:scale-95">
                     <Plus size={20} /> تحرير سند جديد
                  </button>
                  <button onClick={() => onNavigate('acc_reports')} className="px-10 py-5 bg-white/5 border border-white/10 rounded-[1.75rem] font-black text-sm flex items-center gap-3 hover:bg-white/10 transition-all">
                     <BarChart3 size={20} /> التقارير المجمعة
                  </button>
               </div>
            </div>
            <div className="bg-white/5 backdrop-blur-2xl p-10 rounded-[3rem] border border-white/10 shadow-3xl text-center min-w-[300px]">
               <p className="text-[11px] font-black uppercase text-indigo-400 tracking-widest mb-4">إجمالي السيولة المتاحة</p>
               <h2 className="text-6xl font-black tabular-nums mb-4 tracking-tighter">1.28M <span className="text-xl font-bold opacity-30">EGP</span></h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         ...
         <StatCard label="رواتب الكادر" val="85K" icon={<Users />} color="text-blue-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         ...
         <div className="space-y-10">
            <div className="glass-panel p-10 bg-white rounded-[3rem] shadow-sm relative overflow-hidden group">
               ...
               <button onClick={() => onNavigate('acc_reports')} className="w-full mt-10 py-5 bg-slate-950 text-white rounded-2xl font-black text-xs hover:bg-indigo-600 transition-all shadow-xl active:scale-95">استخراج تقرير الحالة المالية</button>
            </div>
         </div>
      </div>
    </div>
