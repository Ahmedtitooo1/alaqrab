...
const ParentView: React.FC<ParentViewProps> = ({ onNavigate, mode = 'overview' }) => {
  const { lang, user, systemName, systemLogo, announcements, allUsers, updateStudentSubscription, addNotification } = useAppContext();
  const isRtl = lang === 'ar';
  ...
  const renderOverview = () => (
    <div className="space-y-10 animate-view">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <SummaryCard label="الاستحقاق المالي القادم" val={isPaid ? "0.00" : student.nextRenewalDate} icon={<CreditCard />} color={isPaid ? "text-emerald-600" : "text-rose-600"} bg={isPaid ? "bg-emerald-50" : "bg-rose-50"} trend={isPaid ? "تم السداد" : "بانتظار الدفع"} />
      </div>

      <div className="space-y-6">
         <div className="flex justify-between items-center px-4">
            <h3 className="text-2xl font-black flex items-center gap-3 text-slate-900"><Megaphone className="text-amber-500" /> آخر التعميمات والأنشطة</h3>
            <button className="text-xs font-black text-indigo-600 uppercase tracking-widest hover:underline">مشاهدة الكل</button>
         </div>
    ...
    <div className="space-y-10 animate-view">
       <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm flex items-center gap-3 shadow-xl shadow-indigo-600/20 hover:scale-105 transition-all">
             <Download size={20} /> تصدير الشهادة الشهرية
          </button>
       </div>

       <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 glass-card p-10 bg-white border border-slate-200 space-y-8">
             <h3 className="text-xl font-black border-b pb-6 flex items-center gap-4 text-slate-900"><FileBadge className="text-indigo-600" size={24} /> سجل أداء الطالب في الاختبارات</h3>
                ...
                {[1, 2, 3].map(id => (
                  <div key={id} className="p-6 bg-slate-50/50 rounded-3xl flex items-center justify-between border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all group">
                     ...
                     <div className="text-left">
                        ...
                        <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest bg-emerald-50 text-emerald-600`}>امتياز</span>
                     </div>
                  </div>
                ))}
             </div>
          ...
          <div className="space-y-10">
             <div className="glass-card p-10 text-center space-y-8 bg-white border shadow-sm">
                <h3 className="text-lg font-black flex items-center justify-center gap-3 text-slate-900"><Activity className="text-rose-600" /> مستوى الالتزام</h3>
                ...
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                   <p className="font-bold text-sm text-emerald-800">حضور متميز والتزام كامل بكافة المواعيد المجدولة.</p>
                </div>
             </div>
          </div>
       </div>
  ...
  const renderAIAnalysis = () => (
    <div className="space-y-10 animate-view">
       <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
             <h2 className="text-3xl font-black text-slate-900 tracking-tight">تحليل الأداء بالذكاء الاصطناعي</h2>
             <p className="text-slate-500 font-bold">رؤية تحليلية مدعومة بذكاء Gemini لمستوى ابنكم الدراسي ومقترحات التحسين.</p>
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex items-start gap-5">
                   ...
                   <div>
                      <h4 className="font-black text-lg text-slate-900">نصيحة اليوم</h4>
                ...
                <div className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm flex items-start gap-5">
                   ...
                   <div>
                      <h4 className="font-black text-lg text-slate-900">تنبؤ المسار</h4>
          ...
          <div className="space-y-10">
             <div className="glass-card p-10 bg-indigo-600 text-white rounded-[3rem] shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-xl font-black mb-6 flex items-center gap-3"><BrainCircuit size={24}/> كيف يعمل المحلل؟</h3>
                <p className="text-sm text-indigo-100 font-bold leading-relaxed mb-8">يقوم المحلل الذكي بمعالجة درجات الطالب، غيابه، ومشاركاته في الحصص المباشرة لتقديم صورة دقيقة وشاملة لولي الأمر.</p>
                <div className="p-4 bg-white/10 rounded-2xl border border-white/20">
                   <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200">مدعوم بـ</p>
    ...
    <div className="space-y-12 animate-view pb-24">
       <div className="flex flex-col md:flex-row justify-between items-center gap-6 no-print">
          <div className="flex items-center gap-6">
             ...
             <div>
                <h2 className="text-3xl font-black text-slate-900 tracking-tight italic uppercase">Subscriptions & Payments</h2>
                <p className="text-slate-500 font-bold">عرض المستحقات، سداد الاشتراكات، وطباعة إيصالات الدفع الرسمية.</p>
             </div>
          </div>
          <div className="px-6 py-3 bg-emerald-50 text-emerald-600 rounded-full text-xs font-black border border-emerald-100 flex items-center gap-3">
             <div className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></div> الحساب نشط مالياً
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="glass-card p-12 space-y-10 border-t-[12px] border-indigo-600 bg-white shadow-2xl no-print relative overflow-hidden">
             ...
             <div className="flex justify-between items-start relative z-10">
                ...
                <div className="text-right">
                   <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">موعد التجديد القادم</p>
                   <p className="text-2xl font-black text-slate-900 tabular-nums">{student.nextRenewalDate || '2024-06-01'}</p>
                </div>
             </div>
             <div className="relative z-10">
                <p className="text-xs font-black text-slate-400 uppercase mb-3 tracking-widest">المبلغ المستحق حالياً</p>
                <h4 className="text-7xl font-black text-slate-900 tabular-nums">{isPaid ? '0.00' : (student.subscriptionAmount || 650).toLocaleString()} <span className="text-2xl font-bold opacity-20">EGP</span></h4>
             </div>
             ...
             <div className="relative z-10 pt-6">
                {!isPaid ? (
                  <button 
                    onClick={() => { 
                      setIsPaid(true); 
                      addNotification({ title: 'عملية دفع ناجحة', content: 'تم سداد الاشتراك وتحديث الحالة المالية للطالب.', type: 'success', date: new Date().toISOString() });
                    }}
                    className="w-full py-7 bg-slate-950 text-white rounded-[2.5rem] font-black text-2xl shadow-3xl hover:scale-[1.02] transition-all flex items-center justify-center gap-4 active:scale-95 group"
                  >
                     <CardIcon size={32} className="group-hover:rotate-12 transition-transform" /> تأكيد الدفع الإلكتروني
                  </button>
                ) : (
                  <div className="p-8 bg-emerald-50 border-4 border-dashed border-emerald-200 rounded-[2.5rem] text-center space-y-5 animate-view">
                     ...
                     <h4 className="text-3xl font-black text-emerald-900">تم التحصيل بنجاح</h4>
                     <p className="font-bold text-emerald-700 text-lg">شكراً لكم! تم تجديد اشتراك الطالب وتوثيق السجل المحاسبي.</p>
                     <button onClick={() => setShowReceipt(true)} className="flex items-center gap-3 mx-auto px-8 py-3 bg-emerald-600 text-white rounded-2xl font-black text-xs shadow-lg hover:bg-emerald-700 transition-colors">
                        <Printer size={18} /> عرض وطباعة إيصال السداد الرسمي
                     </button>
                  </div>
                )}
             </div>
          ...
          <div className="glass-card p-10 space-y-8 bg-white border border-slate-100 no-print shadow-sm">
             <h3 className="font-black text-2xl border-b pb-6 text-slate-900 flex items-center gap-3"><History size={24} className="text-indigo-600" /> سجل المدفوعات التاريخي</h3>
             ...
             <div className="p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                <p className="text-[10px] text-center text-slate-400 font-bold uppercase tracking-widest leading-relaxed">تخضع كافة المعاملات المالية لرقابة المحاسب المالي المعتمد في المنظومة.</p>
             </div>
          </div>
       </div>
            ...
            <div className="glass-card w-full max-w-2xl p-12 bg-white animate-view relative overflow-hidden border-[10px] border-slate-50 rounded-[4rem]">
               ...
               <div className="text-center space-y-8 border-b-2 border-slate-100 pb-12">
                  ...
                  <div className="inline-flex items-center gap-3 px-8 py-3 bg-emerald-50 text-emerald-600 rounded-full font-black text-sm border border-emerald-100 shadow-inner">
                     <div className="w-2 h-2 bg-emerald-500 rounded-full"></div> PAID - تم السداد
                  </div>
               </div>
               ...
               <div className="py-10 space-y-5 text-base font-bold text-slate-600">
                  ...
                  <div className="flex justify-between border-b border-slate-50 pb-3"><span>كود الطالب</span><span className="text-indigo-600 font-black">{student.code}</span></div>
                  <div className="flex justify-between border-b border-slate-50 pb-3"><span>رقم العملية</span><span className="text-slate-950 font-mono tracking-widest uppercase">TX-S-0098224</span></div>
                  <div className="flex justify-between border-b border-slate-50 pb-3"><span>تاريخ السداد</span><span className="text-slate-950">{new Date().toLocaleDateString('ar-EG')}</span></div>
                  <div className="flex justify-between border-b border-slate-50 pb-3"><span>طريقة الدفع</span><span className="text-slate-950">بطاقة بنكية / محفظة إلكترونية</span></div>
                  <div className="flex justify-between pt-10">
                     <span className="text-2xl font-black text-slate-900 uppercase">Total Paid</span>
                     <span className="text-5xl font-black text-indigo-700 tabular-nums">{(student.subscriptionAmount || 650).toLocaleString()} <span className="text-xl">EGP</span></span>
                  </div>
               </div>
               ...
               <div className="flex gap-6 mt-8 no-print">
                  <button onClick={() => window.print()} className="flex-1 py-5 bg-slate-950 text-white rounded-[1.75rem] font-black flex items-center justify-center gap-4 shadow-3xl hover:bg-black active:scale-95 transition-all">
                     <Printer size={24} /> طباعة المستند الرسمي
                  </button>
                  <button onClick={() => setShowReceipt(false)} className="px-10 py-5 bg-slate-100 text-slate-500 rounded-[1.75rem] font-black hover:bg-slate-200 transition-all">إغلاق</button>
               </div>
               
               <p className="text-[10px] text-center text-slate-400 mt-12 italic font-bold leading-relaxed">
                  هذا الإيصال معتمد إلكترونياً وصادر عن منظومة العقرب لإدارة النظم التعليمية. لا يحتاج لتوقيع يدوي أو ختم إضافي ويعد سنداً قانونياً للسداد.
               </p>
            </div>
         </div>
       )}
       ...
       <div className="hidden print:block p-16 bg-white text-right border-[10px] border-black rounded-none min-h-screen">
          ...
          <div className="space-y-10 text-3xl font-bold leading-relaxed">
             <p>نقر بأننا استلمنا من السيد ولي أمر الطالب: <span className="underline font-black">{student.firstName} {student.lastName}</span></p>
             <p>كود الطالب: <span className="underline font-black">{student.code}</span></p>
             <p>مبلغاً وقدره: <span className="text-6xl font-black">{(student.subscriptionAmount || 650).toLocaleString()} جنيه مصري</span> فقط لا غير.</p>
             <p>وذلك عن قيمة: <span className="underline font-black">اشتراك الخدمة التعليمية - شهر يونيو 2024</span></p>
          </div>
          <div className="mt-40 flex justify-between items-end px-12">
             <div className="text-center w-64 border-t-4 border-black pt-4 font-black text-2xl">توقيع الموظف المستلم</div>
             <div className="text-center w-64 border-t-4 border-black pt-4 font-black text-2xl">ختم المنظومة الرقمي</div>
          </div>
  ...
  <div className="flex items-center justify-between p-6 bg-slate-50/50 rounded-[2.5rem] border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all group shadow-xs">
     <div className="flex items-center gap-5">
        ...
        <div>
           ...
           <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Transaction ID: {refNo}</p>
        </div>
     </div>
     <div className="flex items-center gap-8">
        <span className="font-black text-xl text-slate-700 tabular-nums">{amount}</span>
        <button className="p-4 bg-white border border-slate-100 rounded-2xl text-slate-400 group-hover:text-indigo-600 transition-all shadow-sm hover:scale-110" title="تحميل الإيصال"><Download size={20} /></button>
     </div>
  </div>
);
