...
import { Sparkles, BrainCircuit, Lightbulb, Zap, CheckCircle, RefreshCw, Edit2, ArrowLeftRight, Loader2, ChevronDown, ChevronUp, BookOpen } from 'lucide-react';
import { useAppContext } from '../context/AppContext';
import { UserRole } from '../types';
import { getSmartAnalysis, generateRemedialQuiz } from '../services/geminiService';
...
const SmartAnalytic: React.FC<SmartAnalyticProps> = ({ role, dataContext }) => {
  ...
  return (
    <div className={`relative rounded-[2.5rem] border-2 overflow-hidden transition-all duration-500 shadow-2xl flex flex-col backdrop-blur-xl ${theme === 'dark'
                    ...
                    {(role === UserRole.STUDENT || role === UserRole.PARENT) && (
                      <button
                        onClick={async () => {
                          ...
                          try {
                            await generateRemedialQuiz(analysis);
                            alert(isRtl ? "تم إرسال الاختبار العلاجي إلى قائمة الاختبارات!" : "Remedial Quiz sent to exams list!");
                          } catch (e) {
                            alert(isRtl ? "حدث خطأ أثناء توليد الاختبار" : "Error generating quiz");
                          } finally {
                            setQuizGenerating(false);
                          }
                        }}
                        disabled={quizGenerating}
                        className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-[11px] font-black shadow-lg hover:bg-indigo-700 transition-all disabled:opacity-50">
                        {quizGenerating ? <Loader2 size={14} className="animate-spin" /> : <BookOpen size={14} />}
                        {isRtl ? 'إنشاء اختبار علاجي' : 'Create Remedial Quiz'}
                      </button>
                    )}
                    {(role !== UserRole.STUDENT && role !== UserRole.PARENT) && (
