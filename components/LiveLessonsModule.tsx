...
const LiveLessonsModule: React.FC<{ isStudentView?: boolean }> = ({ isStudentView = false }) => {
   ...
   // في النسخة التجريبية: سنفترض أن المعلم بدأ بثاً إذا كان العرض للطالب
   const [isTeacherLive, setIsTeacherLive] = useState(isStudentView);
   const [sessionStatus, setSessionStatus] = useState<'idle' | 'starting' | 'active' | 'requesting'>('idle');
   ...
   const approveMic = (requestId: string) => {
      const student = studentRequests.find(r => r.id === requestId);
      if (student) {
         setOpenMics([...openMics, { id: student.id, name: student.name, status: 'unmuted' }]);
         setStudentRequests(studentRequests.filter(r => r.id !== requestId));
      }
   };

   // واجهة الطالب عند عدم الانضمام ولكن المعلم "بث"
         ...
         <div className="max-w-4xl mx-auto py-16 animate-view">
            <div className="glass-panel p-16 bg-white rounded-[4rem] shadow-3xl border-none text-center space-y-10 relative overflow-hidden">
               ...
               <div className="space-y-6">
                  ...
                  <h2 className="text-5xl font-black text-slate-900 tracking-tight">المعلم "محمود العزازي" يبث الآن!</h2>
                  <p className="text-slate-500 font-bold text-2xl max-w-xl mx-auto leading-relaxed">
                     بدأت حصة "المراجعة النهائية - الفصل الخامس" منذ ٥ دقائق. انضم الآن لتتمكن من التفاعل والمشاركة.
                  </p>
               </div>
               ...
               <div className="pt-10">
                  <button
                     ...
                     className="px-20 py-8 bg-indigo-600 text-white rounded-[2.5rem] font-black text-3xl shadow-3xl hover:bg-indigo-700 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-6 mx-auto group"
                  >
                     <PlayCircle size={48} className="group-hover:rotate-12 transition-transform" /> انضمام للبث المباشر
                  </button>
                  <p className="mt-8 text-slate-400 font-bold text-sm italic">سيتم تسجيل حضورك تلقائياً فور الانضمام</p>
               </div>
            </div>
         </div>
      );
   ...
   if (isJoined) {
      return (
         <div className="fixed inset-0 z-[1000] bg-slate-950 flex flex-col lg:flex-row animate-view overflow-hidden no-print">
            ...
            <div className="flex-1 relative flex flex-col bg-black">
               <div className="absolute top-8 right-8 z-20 flex gap-4">
                  <div className="px-6 py-2 bg-rose-600 text-white rounded-full text-[10px] font-black uppercase flex items-center gap-3 shadow-3xl animate-pulse">
                     <Radio size={18} /> {isStudentView ? 'مستمع' : 'مباشر الآن'}
                  </div>
               ...
               <div className="flex-1 flex items-center justify-center relative group">
                  {/* View Area (Simulated Camera/Screen Share) */}
                  <div className={`w-full h-full bg-gradient-to-t from-slate-950 to-slate-900 flex items-center justify-center transition-all`}>
                     <div className="text-center space-y-4">
                        <div className="w-72 h-72 bg-white/5 rounded-full flex items-center justify-center border-4 border-white/10 overflow-hidden relative shadow-3xl">
                        ...
                        <h2 className="text-4xl font-black text-white">م/ محمود العزازي</h2>
                        <p className="text-indigo-400 font-black uppercase tracking-[0.3em] text-sm">التفاعل مع الطلاب نشط</p>
                     </div>
                  </div>
                  ...
                  <div className="absolute bottom-10 inset-x-0 mx-auto w-fit bg-slate-900/90 backdrop-blur-3xl border border-white/10 p-5 rounded-[3rem] flex items-center gap-6 shadow-3xl opacity-0 group-hover:opacity-100 transition-all duration-500 scale-95 group-hover:scale-100">
                     ...
                     {!isStudentView && (
                        <button className="p-5 bg-white/10 text-white rounded-2xl hover:bg-white/20 transition-all">
                           <MonitorUp size={24} />
                        </button>
                     )}
                     {isStudentView && (
                     ...
                     <button onClick={() => { setIsJoined(false); setIsTeacherLive(false); }} className="px-10 py-5 bg-rose-600 text-white rounded-[1.75rem] font-black text-sm flex items-center gap-3 shadow-xl active:scale-95">
            ...
            <div className="w-full lg:w-[450px] bg-slate-900 border-r border-white/5 flex flex-col shadow-2xl">
               <div className="p-8 border-b border-white/5 bg-slate-950 flex justify-between items-center flex-row-reverse">
                  <h3 className="font-black text-lg flex items-center gap-4 uppercase tracking-tighter text-white">
                     <MessageSquare className="text-indigo-500" /> مركز التفاعل
                  </h3>
                  <span className="px-3 py-1 bg-white/5 text-slate-400 rounded-lg text-[10px] font-black">240 ONLINE</span>
               </div>

               <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar text-right">
                  {/* Student Requests to talk */}
                  {!isStudentView && studentRequests.length > 0 && (
                     <div className="space-y-4 animate-bounce-slow">
                        <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-widest flex items-center justify-end gap-2">طلبات التحدث المباشر <Hand size={14} /></h4>
                        {studentRequests.map(req => (
                           <div key={req.id} className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center justify-between">
                              <button onClick={() => approveMic(req.id)} className="px-4 py-2 bg-amber-500 text-slate-900 rounded-xl font-black text-[10px]">موافقة</button>
                  ...
                  <div className="space-y-4">
                     <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center justify-end gap-2">المتحدثون الآن <Mic size={14} /></h4>
                     {openMics.map(m => (
                        <div key={m.id} className="p-4 bg-white/5 rounded-2xl flex items-center justify-between">
                           {!isStudentView && m.id !== 'u10' ? (
                              <button onClick={() => setOpenMics(openMics.filter(x => x.id !== m.id))} className="p-2 text-rose-500 hover:bg-rose-500/10 rounded-lg"><VolumeX size={16} /></button>
                           ) : <div></div>}
                           <div className="flex items-center gap-3">
                              <span className="text-xs font-bold text-white">{m.name}</span>
                              <div className={`w-2 h-2 rounded-full animate-pulse ${m.status === 'unmuted' ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>
                           </div>
                        </div>
                     ))}
                  </div>
                  ...
                  {/* Chat Content */}
                  <div className="space-y-6">
                     <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center justify-end gap-2">التعليقات والأسئلة <MessageSquare size={14} /></h4>
                     {comments.map(c => (
                        <div key={c.id} className="space-y-2 animate-view text-right">
                           <div className="flex justify-between items-center text-[9px] font-black text-slate-500 flex-row-reverse">
                              <span className="text-indigo-400">@{c.user}</span>
                              <span className="font-mono">{c.time}</span>
                           </div>
                           <p className={`text-xs font-bold p-4 bg-white/5 rounded-2xl border border-white/5 text-right text-white`}>
                              {c.text}
                           </p>
                        </div>
                     ))}
               ...
               <div className="p-6 bg-slate-950 border-t border-white/5 flex gap-4 flex-row-reverse">
                  <input placeholder="اسأل المعلم شيئاً..." className="flex-1 bg-white/5 border-none outline-none p-4 rounded-2xl text-xs font-bold text-white shadow-inner focus:ring-1 focus:ring-indigo-600 text-right" />
